...
export class AltaAlumnoComponent implements OnInit , OnDestroy{
  ...
  ngOnInit(): void {
    ...
    this.cursosSub = this.curso$.subscribe((curso : Curso[]) => this.cursos = curso)
  ...
  verCurso(){
   ...
   bajaCurso(curso : string){
    let cursoEncontrado: Curso | undefined | any=  this.cursos.find(el => el.nombreCurso == curso)
     cursoEncontrado.inscriptos = cursoEncontrado?.inscriptos.filter((el: Alumno) => el.nameUsuario != this.sesion.usuarioActivo?.usuario)
